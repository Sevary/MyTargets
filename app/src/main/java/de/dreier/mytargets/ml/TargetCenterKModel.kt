...
import de.dreier.mytargets.ml.TargetCenterK


class TargetCenterKModel(context: Context) {
    private val model: TargetCenterK = TargetCenterK.newInstance(context)
    private val imgSize = 480
    ...
    fun process(bitmap: Bitmap): List<Detection> {
        ...
        for (i in 0 until num) {
            val base = i * stride
            // assumed ordering: [xMin, yMin, xMax, yMax, score, class, kpX, kpY]
            val xMin = out[base + 0]
            val yMin = out[base + 1]
            val xMax = out[base + 2]
            val yMax = out[base + 3]
            ...
            val keypoint: PointF? = if (stride >= 8) {
                val kx = out[base + 6]
                val ky = out[base + 7]
                PointF(kx, ky)
            } else {
            ...
            detections.add(Detection(score, rect, cls, keypoint, null))
        }
